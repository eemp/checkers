...
Game.prototype.apply_move = function(move_info) {
    ...
    if(!this._jump_again) self.increment_turn();
}

Game.prototype._get_available_moves = function() {
    var self = this;
...
Game.prototype.get_available_moves = function() {
    ...
    if(self._jump_from_loc) {
        ...
        availableMoves = [];
        var player = self.turn();
        var piecesLocations = Object.keys(self.boardObj()._pieces[player]);
        
        for(var k = 0; k < piecesLocations.length; k++) {
            var loc = JSON.parse(piecesLocations[k]);
            var x = loc[0],
                y = loc[1];
            var movesForPiece = self.get_available_moves_from_stp(x, y);
            if(movesForPiece.length > 0) {
...
function compareLocArrays(l1, l2) {
    l1 = l1.sort();
    l2 = l2.sort();
...
Game.prototype.hash = function(d) {
    d = d > 8 ? 8 : 
        d > 5 ? 5 :
        d > 3 ? 3 :
        1;
    // return d + "," + this.turn + ',' + this._board.hash();
    return this.turn + ',' + this._board.hash();
}
...
Game.prototype.clone = function() {
    ...
    clone._gameStatus = self._gameStatus;
    clone._turn = self._turn;
    clone._board = self._board.clone();
    clone._jump_again = self._jump_again;
    clone._jump_from_loc = self._jump_from_loc ? self._jump_from_loc.slice() : null;
